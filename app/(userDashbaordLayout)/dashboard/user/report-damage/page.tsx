...
import { useGetMyReportsQuery } from "@/redux/api/subscription/subscriptionApi";
...
const ReportDamagePage = () => {
    ...
    const { data: reportsResponse, isFetching } = useGetMyReportsQuery({
        page: currentPage,
        limit: limit,
        sortBy: sortBy,
        ...(status !== "All" && { status }),
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-gray-50/50 border-y border-gray-100">
                                <th className="px-6 py-4 text-center whitespace-nowrap text-gray-400 font-bold text-xs uppercase tracking-wider">Report ID</th>
                                <th className="px-6 py-4 text-center whitespace-nowrap text-gray-400 font-bold text-xs uppercase tracking-wider">Type</th>
                                ...
                                reports.map((report: any, index: number) => (
                                    <tr key={index} className="hover:bg-gray-50/50 transition-colors group">
                                        <td className="px-6 py-6 whitespace-nowrap text-[#172C41] font-bold text-sm uppercase">
                                            #{report.reportId || report.id.slice(-6)}
                                        </td>
                                        ...
                                        <td className="px-6 py-6 whitespace-nowrap text-right">
                                            <button
                                                onClick={() => {
                                                    setSelectedRequestId(report.id);
