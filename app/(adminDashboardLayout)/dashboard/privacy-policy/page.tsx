...
const PrivacyPolicyPage = () => {
  const { data: privacyData, isLoading: isFetching } = useGetAllPrivacyQuery();
  const [createPrivacy, { isLoading: isUpdating }] = useCreatePrivacyMutation();
  const [editorContent, setEditorContent] = useState("");

  useEffect(() => {
    if (privacyData?.data?.content) {
      setEditorContent(privacyData.data.content);
    }
  }, [privacyData]);

  const handleSave = async () => {
    try {
      await createPrivacy({ content: editorContent }).unwrap();
  ...
  return (
    <Container>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <h1 className="text-2xl font-bold text-[#0A2540] font-primary">Privacy Policy</h1>
        <Button 
          onClick={handleSave}
          disabled={isUpdating}
