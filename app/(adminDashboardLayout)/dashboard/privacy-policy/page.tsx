...
// Mock Data
const initialPrivacyPolicies = Array.from({ length: 12 }, (_, i) => ({
  id: (i + 1).toString(),
...
const PrivacyPolicyPage = () => {
  ...
  const [policies, setPolicies] = useState(initialPrivacyPolicies);
  const [searchTerm, setSearchTerm] = useState("");
  const [editingPolicy, setEditingPolicy] = useState<(typeof initialPrivacyPolicies)[0] | null>(null);
  ...
  const handleAddPolicy = () => {
    ...
    setPolicies([...policies, { id, content: editorContent }]);
    setView("list");
  };

  const handleUpdatePolicy = () => {
    if (!editingPolicy) return;
    setPolicies(policies.map(p => p.id === editingPolicy.id ? { ...p, content: editorContent } : p));
    setView("list");
  };
      ...
      <Container>
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-[#0A2540]">
            {view === "add" ? "Add New Privacy Policy" : "Edit Privacy Policy"}
          </h1>
        </div>
        ...
        <div className="bg-white p-8 md:p-12 shadow-sm border border-gray-100">
          <div className="space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <Button 
                onClick={view === "add" ? handleAddPolicy : handleUpdatePolicy}
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold text-[#0A2540] mb-6 font-primary">Privacy Policy</h1>
      ...
      <div className="bg-white shadow-sm border border-gray-100 mb-8 divide-y divide-gray-100">
        {filteredPolicies.length > 0 ? (
          filteredPolicies.map((policy, index) => (
            <div key={policy.id} className="p-6 flex items-start justify-between gap-4 hover:bg-gray-50/20 transition-colors group">
